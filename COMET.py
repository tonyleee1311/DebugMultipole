...
def find_good_multipoles_complete_parallel(cliques_gf, CorrMat, sigma, delta, group_sz):
    ...
    for i, (x) in enumerate(AllGroupOutput):
        for k, (a,b,c,d) in enumerate(x):
            AllMPs += [a]
            AllLEVs += [b]
            AllLEVGs += [c]
            AllSizes += [d]
...
def COMET_EXT(CorrMat, sigma, delta, edge_filt, group_sz):
    ...
    t1 = time.time()
    #    group_sz = 50000
    [AllMPs, AllLEVs, AllLEVGs, AllSizes] = find_good_multipoles_complete_parallel(cliques_gf, CorrMat, sigma, delta,
                                                                                   group_sz)
    t2 = time.time()
    ...
    t1 = time.time()
    [FinalMPList, FinalLEVList, FinalLEVGList, FinalSzList] = MISC.remove_redundant_multipoles_alter_parallel(AllMPs,
                                                                                                              AllLEVs,
                                                                                                              AllLEVGs)
